...
type State = {
  ...
  doodads: DoodadItem[];
  walls: WallItem[];
  activeCategory: "borders" | "grounds" | "doodads" | "walls";
  activeItemId: string | null;
};

type Action =
  | { type: "SET_CATEGORY"; category: "borders" | "grounds" | "doodads" | "walls" }
  ...
  | { type: "CLEAR_CATEGORY"; category: "borders" | "grounds" | "doodads" | "walls" };
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case "DELETE_BORDER":
      return {
        ...state,
        borders: state.borders.filter((b) => b.id !== action.id),
    ...
    case "DELETE_GROUND":
      return {
        ...state,
        grounds: state.grounds.filter((g) => g.id !== action.id),
        activeItemId: state.activeItemId === action.id ? null : state.activeItemId,
      };
    ...
    case "DELETE_DOODAD":
      return {
        ...state,
        doodads: state.doodads.filter((d) => d.id !== action.id),
        activeItemId: state.activeItemId === action.id ? null : state.activeItemId,
      };
